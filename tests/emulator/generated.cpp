...
{
    uint64_t op;
    bool operator==(const oneop_result& o) const { return op == o.op; }
};
...
{
    uint64_t src, dst;
    bool operator==(const twoop_result& o) const { return src == o.src and dst == o.dst; }
};
...
TEST_CASE(manual_test_instruction)
{
    ...
    struct result
    {
        ...
        result(uint64_t v1, uint64_t v2) : op1_16(v1), op2_16(v2), op1_64(v1), op2_64(v2) {}
    ...
    for (const auto v1 : values) {
        for (const auto v2 : values) {
            ...
            asm volatile(
                ...
                "pushf;"
                "pop %[flags_emul_64];"
                : [op1_native_16] "+r"(result_native.op1_16), [op2_native_16] "+r"(result_native.op2_16),
                  [op1_emul_16] "+r"(result_emul.op1_16), [op2_emul_16] "+r"(result_emul.op2_16),
                  [op1_native_64] "+r"(result_native.op1_64), [op2_native_64] "+r"(result_native.op2_64),
                  [op1_emul_64] "+r"(result_emul.op1_64), [op2_emul_64] "+r"(result_emul.op2_64),
                  [flags_native_16] "=rm"(result_native.flags_16), [flags_native_64] "=rm"(result_native.flags_64),
                  [flags_emul_16] "=rm"(result_emul.flags_16), [flags_emul_64] "=rm"(result_emul.flags_64)
