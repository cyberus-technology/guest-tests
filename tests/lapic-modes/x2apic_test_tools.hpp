...
struct x2apic_msr
{
    ...
    bool is_readable() const { return (mode & READ) == READ; };
    bool is_writeable() const { return (mode & WRITE) == WRITE; };

    bool is_32bit() const { return width == BITS_32; }
    bool is_64bit() const { return width == BITS_64; }

    bool is_preserved_on_init() const { return preserved == PRESERVED_ON_INIT; };

    uint64_t read() const { return rdmsr(addr); }
    void write(uint64_t value) { wrmsr(addr, value); }
...
public:
    ...
    explicit mmio_buffer_t(x2apic_msr_table_t& msr_table_) : msr_table(msr_table_) {}
...
struct x2apic_mode_guard
{
    ...
    ~x2apic_mode_guard() { x2apic_mode_disable(); }
};
