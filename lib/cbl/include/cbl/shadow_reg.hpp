...
public:
    ...
    static void free_resource_impl(resource_t& res) { res = nullptr; }
...
public:
    ...
    void free_resource() { free_resource_impl(host_resource_); }
    ...
    void read(cbl::buffer_view& buf, off_t offset) const
    {
        ...
        case 1: buf.write<uint8_t>(shadow_buf.read<const uint8_t>(offset)); break;
        case 2: buf.write<uint16_t>(shadow_buf.read<const uint16_t>(offset)); break;
        case 4: buf.write<uint32_t>(shadow_buf.read<const uint32_t>(offset)); break;
        case 8: buf.write<uint64_t>(shadow_buf.read<const uint64_t>(offset)); break;
        }
    }
    ...
    void set_shadow(const cbl::buffer_view& buf, off_t offset)
    {
        ASSERT(sizeof(reg_t) >= buf.bytes() + offset,
               "Invalid access on shadow_reg regsz: {#x} bytes: {#x} offset: {#x}", sizeof(reg_t), buf.bytes(), offset);
        ...
        case 1: shadow_buf.write<uint8_t>(buf.read<const uint8_t>(), offset); break;
        case 2: shadow_buf.write<uint16_t>(buf.read<const uint16_t>(), offset); break;
        case 4: shadow_buf.write<uint32_t>(buf.read<const uint32_t>(), offset); break;
        case 8: shadow_buf.write<uint64_t>(buf.read<const uint64_t>(), offset); break;
        }
    }
