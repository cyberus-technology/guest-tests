...
protected:
    ...
    bool compare_exchange_internal(T expected, T desired, std::memory_order success, std::memory_order failure,
                                   bool weak)
    {
...
public:
    explicit in_place_atomic_base(T& v) : v_(&v) {}
    ...
    bool is_lock_free() { return __atomic_is_lock_free(sizeof(T), v_); }
    ...
    void store(T desired, std::memory_order order = std::memory_order_seq_cst) { __atomic_store(v_, &desired, order); }
};
...
public:
    ...
    T operator+=(T v) { return fetch_add(v); }
    T operator-=(T v) { return fetch_sub(v); }
    T operator&=(T v) { return fetch_and(v); }
    T operator|=(T v) { return fetch_or(v); }
    T operator^=(T v) { return fetch_xor(v); }
};
...
public:
    ...
    T* operator+=(std::ptrdiff_t v) { return fetch_add(v); }
    T* operator-=(std::ptrdiff_t v) { return fetch_sub(v); }
};
