...
public:
    ...
    public:
        ...
        interval get_ivals(uint2type<1>) const { return {base[0], base[0] + current_step}; }
        ...
        auto operator*() const { return get_ivals(uint2type<N> {}); }
    ...
    iterator begin() const { return {base, size, max_order}; }
    iterator end() const
    {
        auto bs(base);
        for (auto& b : bs) {
