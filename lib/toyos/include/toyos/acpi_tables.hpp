...
struct acpi_rsdt : acpi_table_header
{
    ...
    size_t number_of_entries() const { return (length - sizeof(acpi_rsdt)) / sizeof(uint32_t); }
    uint32_t entry(size_t idx) const { return desc_base[idx]; }
};
#pragma pack(pop)
...
struct acpi_mcfg : acpi_table_header
{
    ...
    size_t busses() const { return bus_end - bus_start + 1; } ///< Returns the number of busses in this segment.
};
#pragma pack(pop)
...
public:
    ...
    const acpi_remap& operator*() const { return *reinterpret_cast<acpi_remap*>(cur_remap); }

    bool operator!=(const dmar_remapping_iterator&) const { return cur_remap < end_addr; }
...
struct acpi_dmar : acpi_table_header
{
    ...
    {
    public:
        explicit dmar_remapping_list(const acpi_dmar& dmar_) : dmar(dmar_) {}
    ...
    dmar_remapping_list remapping_entries() const { return dmar_remapping_list(*this); }
};
...
struct acpi_drhd : acpi_remap
{
    ...
    bool is_catchall() const { return flags & 1; }
