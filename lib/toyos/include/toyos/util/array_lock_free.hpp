...
public:
     ...
     */
    value_type at(size_type pos) { return array.at(pos).load(); }
    ...
    std::optional<size_type> find_if(FUNC cond) noexcept
    {
        auto it {std::find_if(array.begin(), array.end(),
                              [cond](const std::atomic<T>& array_value) { return cond(array_value.load()); })};
        if (it != array.end()) {
     ...
     */
    void clear() { std::fill(array.begin(), array.end(), INVALID_ELEMENT); }
     ...
     */
    size_type size() { return array.size(); }
...
public:
    array_lock_free_with_remove() : parent_array() {}
