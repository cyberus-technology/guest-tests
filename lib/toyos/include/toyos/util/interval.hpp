...
struct interval_impl
{
    ...
    constexpr interval_impl(ival_t begin_, ival_t end_) : a {begin_}, b {end_} {}

    /// Constructs an empty interval
    constexpr interval_impl() : interval_impl<ival_t>(ival_t {}, ival_t {}) {}

    /// Constructs an interval from base and size
    static constexpr interval_impl<ival_t> from_size(ival_t base, ival_t size) { return {base, base + size}; }

    /// Constructs an interval which only contains one point
    static constexpr interval_impl<ival_t> from_point(ival_t p) { return from_size(p, ival_t {1}); }
    ...
    constexpr bool empty() const { return b <= a; }

    /// Returns the size. The size of invalid intervals is 0.
    constexpr ival_t size() const { return empty() ? ival_t {0} : b - a; }
    ...
    constexpr bool contains(const ival_t& v) const { return not empty() and a <= v and b > v; }
    ...
    constexpr bool operator==(const interval_impl<ival_t>& o) const { return a == o.a && b == o.b; }

    /// Returns true if given interval does not equal the other interval
    constexpr bool operator!=(const interval_impl<ival_t>& o) const { return not this->operator==(o); }

    /// Returns true if interval ends before other interval begins
    ///
    /// Do not use on empty/invalid intervals.
    constexpr bool operator<(const interval_impl<ival_t>& o) const { return b <= o.a; }
    ...
    public:
        ...
        constexpr iterator(ival_t start) : pos {start} {}

        constexpr ival_t operator*() const { return pos; }
        ...
        constexpr ival_t operator-(const iterator& o) const { return pos - o.pos; }
    ...
    constexpr iterator end() const { return {b}; }
};
...
struct strided_interval_impl
{
    ...
    struct iterator
    {
        ...
        constexpr iterator(ival_t pos_, ival_t stride_) : pos(pos_), stride(stride_) {}
        ...
        constexpr bool operator!=(const iterator& o) const { return not operator==(o); }

        constexpr bool operator<(const iterator& o) const { return pos < o.pos; }
    };

    constexpr iterator begin() const { return {ival.a, stride}; }
    constexpr iterator end() const { return {ival.b, stride}; }
    constexpr std::reverse_iterator<iterator> rbegin() const { return std::make_reverse_iterator(end()); }
    constexpr std::reverse_iterator<iterator> rend() const { return std::make_reverse_iterator(begin()); }
};
