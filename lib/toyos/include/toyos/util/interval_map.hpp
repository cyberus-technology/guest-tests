...
public:
    ...
    interval_vector_impl(value_t val = value_t {}) : m {pair_t {ival_t {}, val}} {}
    ...
    {
        interval_range(const cit_t& b, const cit_t& e) : begin_(b), end_(e) {}

        const cit_t begin_;
        const cit_t end_;

        cit_t begin() const { return begin_; }
        cit_t end() const { return end_; }
    };
    ...
    auto begin() const { return std::begin(m); }
    auto end() const { return std::end(m); }

    auto& internal_map() { return m; }
    const auto& internal_map() const { return m; }
