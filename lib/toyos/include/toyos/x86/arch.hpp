...
public:
   ...
   explicit constexpr generic_addr_t(uintptr_t addr_)
      : addr(addr_) {}
...
{
public:
   explicit constexpr page_num_t(uint64_t page_number_ = 0)
      : generic_addr_t(page_number_) {}

   static constexpr page_num_t from_address(uint64_t addr_)
   {
      return page_num_t{ addr2pn(addr_) };
   }

   constexpr page_num_t operator+(const page_num_t& o) const
   {
      return page_num_t{ this->addr + o.addr };
   }
   constexpr page_num_t operator-(const page_num_t& o) const
   {
      return page_num_t{ this->addr - o.addr };
   ...
   constexpr bool operator<(const page_num_t& o) const
   {
   ...
   constexpr uint64_t to_address() const
   {
      return pn2addr(addr);
   }
...
{
public:
   explicit constexpr lin_addr_t(uintptr_t addr_)
      : generic_addr_t(addr_) {}

   page_num_t pn()
   {
      return page_num_t::from_address(this->addr);
   }

   lin_addr_t operator+(size_t bytes) const
   {
      return lin_addr_t(addr + bytes);
   }

   bool operator<(const lin_addr_t& rhs) const
   {
...
{
public:
   explicit constexpr phy_addr_t(uintptr_t addr_)
      : generic_addr_t(addr_) {}
   ...
   bool operator<(const phy_addr_t& o) const
   {
      return addr < o.addr;
   }
   bool operator>(const phy_addr_t& o) const
   ...
   phy_addr_t operator+(size_t bytes) const
   {
      return phy_addr_t(addr + bytes);
   }
   phy_addr_t operator-(size_t bytes) const
   {
      return phy_addr_t(addr - bytes);
   }
   phy_addr_t operator-(phy_addr_t other) const
   {
      return phy_addr_t(addr - other.addr);
   }
};

struct log_addr_t
{
   explicit log_addr_t(x86::segment_register seg_, uintptr_t off_)
      : seg(seg_), off(off_) {}

   log_addr_t() = delete;

   log_addr_t operator+(size_t bytes) const
   {
      return log_addr_t(seg, off + bytes);
   }
   log_addr_t operator-(size_t bytes) const
   {
      return log_addr_t(seg, off - bytes);
   ...
   bool operator==(const log_addr_t& o) const
   {
      return seg == o.seg and off == o.off;
   }
   bool operator!=(const log_addr_t& o) const
   {
      return not operator==(o);
