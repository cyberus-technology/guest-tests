...
struct gdt_segment
{
    ...
    bool ar_set(const ar_flag_mask flag) const { return (ar & to_underlying(flag)) > 0; }
};
...
struct __PACKED__ descriptor_ptr
{
    uint16_t limit;
    uint64_t base;
    bool operator!=(const descriptor_ptr& rhs) const { return not(base == rhs.base and limit == rhs.limit); }
};
...
struct segment_selector
{
    ...
    explicit segment_selector(uint16_t raw_) : raw(raw_) {}

    uint16_t index() const { return (raw >> INDEX_SHIFT) & math::mask(INDEX_WIDTH); };

    uint16_t value() const { return raw; }
...
struct gdt_entry
{
    ...
    bool g() const { return ar() & AR_G; }
    bool system() const { return not(ar() & AR_S); } // System segments have S=0
    bool present() const { return ar() & AR_P; }
    bool is_code() const { return (not system()) and (ar() & AR_X); }
    bool is_conforming() const { return is_code() and (ar() & AR_CE); }
    uint8_t dpl() const { return (ar() & AR_DPL) >> AR_DPL_SHIFT; }
    segment_type type() const { return static_cast<segment_type>(ar() & math::mask(AR_TYPE_WIDTH)); }
    ...
    void set_type(segment_type t) { set_ar((ar() & ~math::mask(AR_TYPE_WIDTH)) | to_underlying(t)); }
    ...
    void set_g(bool g) { set_ar(set_bit(ar(), g, AR_G_SHIFT)); }
    void set_system(bool s) { set_ar(set_bit(ar(), not s, AR_S_SHIFT)); } // System segments have S=0
    void set_present(bool p) { set_ar(set_bit(ar(), p, AR_P_SHIFT)); }
    void set_db(bool db) { set_ar(set_bit(ar(), db, AR_DB_SHIFT)); }
