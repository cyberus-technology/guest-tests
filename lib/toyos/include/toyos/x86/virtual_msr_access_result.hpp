...
public:
   static virtual_msr_access_result access_succeeded_with(uint64_t val)
   {
      return virtual_msr_access_result(val);
   }

   static virtual_msr_access_result access_succeeded()
   {
      return virtual_msr_access_result(0);
   }
   ...
   {
      return res_ == msr_result::SUCCESS;
   }
   bool caused_exception() const
   {
      return res_ == msr_result::EXCEPTION;
   }
   bool was_not_handled() const
   {
      return res_ == msr_result::UNHANDLED;
   }
