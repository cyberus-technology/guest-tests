...
public:
     ...
     */
    void set_rev_id(uint32_t rev_id_) { raw.rev_id = rev_id_; }
     ...
     * \return value
     */
    uint64_t read(x86::vmcs_encoding enc) { return vmread(enc); }
     ...
     */
    void write(x86::vmcs_encoding enc, uint64_t val) { vmwrite(enc, val); }
...
public:
    ...
    uint8_t get_type() const { return get_bits(TYPE_SHIFT, TYPE_WIDTH); }
    void set_type(uint8_t v) { set_bits(v, TYPE_SHIFT, TYPE_WIDTH); }

    bool get_system() const { return get_bits(SYSTEM_SHIFT); }
    void set_system(bool v) { set_bits(v, SYSTEM_SHIFT); }

    uint8_t get_dpl() const { return get_bits(DPL_SHIFT); }
    void set_dpl(bool v) { set_bits(v, DPL_SHIFT); }

    bool get_present() const { return get_bits(PRESENT_SHIFT); }
    void set_present(bool v) { set_bits(v, PRESENT_SHIFT); }

    bool get_avl() const { return get_bits(AVL_SHIFT); }
    void set_avl(bool v) { set_bits(v, AVL_SHIFT); }

    bool get_cs_long() const { return get_bits(CS_LONG_SHIFT); }
    void set_cs_long(bool v) { set_bits(v, CS_LONG_SHIFT); }

    bool get_size() const { return get_bits(SIZE_SHIFT); }
    void set_size(bool v) { set_bits(v, SIZE_SHIFT); }

    bool get_granularity() const { return get_bits(GRANULARITY_SHIFT); }
    void set_granularity(bool v) { set_bits(v, GRANULARITY_SHIFT); }

    bool get_unusable() const { return get_bits(UNUSABLE_SHIFT); }
    void set_unusable(bool v) { set_bits(v, UNUSABLE_SHIFT); }

    uint32_t get_value() const { return raw; }
...
private:
    ...
    uint32_t get_bits(uint32_t shift, uint32_t width = 1) const { return (raw >> shift) & math::mask(width); }
...
public:
     ...
     */
    cached_segment_descriptor(x86::descriptor_ptr gdtr, uint16_t selector_) : selector(selector_)
    {
