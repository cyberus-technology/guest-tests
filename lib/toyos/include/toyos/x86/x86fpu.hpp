...
{
    xmm_t ret;
    asm volatile("movdqu %%xmm0, %0" : "=m"(ret));
    return ret;
}
...
{
    zmm_t ret;
    asm volatile("vmovdqu64 %%zmm0, %0" : "=m"(ret));
    return ret;
}
...
{
    zmm_t ret;
    asm volatile("vmovdqu64 %%zmm23, %0" : "=m"(ret));
    return ret;
}
...
{
    uint64_t ret;
    asm volatile("kmovq %%k0, %0" : "=rm"(ret));
    return ret;
}
...
{
    asm volatile("fxsave %0" : "=m"(*store)::"memory");
}

inline void fxrstor(uint8_t* store)
{
    asm volatile("fxrstor %0" ::"m"(*store) : "memory");
}
...
{
    asm volatile("xrstor %[area]" ::"a"(features & math::mask(32)), "d"(features >> 32), [area] "m"(*store) : "memory");
}
