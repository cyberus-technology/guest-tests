...
struct mbi2_tag
{
    ...
    explicit mbi2_tag(uint32_t t) : type(t) {}
    mbi2_tag(uint32_t t, uint32_t s) : type(t), size(s) {}
...
struct mbi2_boot_module : mbi2_tag
{
    ...
    mbi2_boot_module(uint32_t s, uint32_t e) : mbi2_tag(TYPE), start(s), end(e) {}
    mbi2_boot_module() : mbi2_boot_module(0, 0) {}
...
struct mbi2_efi_system_table : mbi2_tag
{
    ...
    explicit mbi2_efi_system_table(uint64_t system_table_) : mbi2_tag(TYPE), system_table(system_table_) {}

    mbi2_efi_system_table() : mbi2_efi_system_table(0) {}
...
struct mbi2_efi_image_handle : mbi2_tag
{
    ...
    explicit mbi2_efi_image_handle(uint64_t image_handle_) : mbi2_tag(TYPE), image_handle(image_handle_) {}

    mbi2_efi_image_handle() : mbi2_efi_image_handle(0) {}
...
struct mbi2_efi_boot_services : mbi2_tag
{
    ...
    mbi2_efi_boot_services() : mbi2_tag(TYPE) {}
};
...
struct mbi2_image_load_base : mbi2_tag
{
    ...
    mbi2_image_load_base() : mbi2_tag(TYPE) {}

    explicit mbi2_image_load_base(uint32_t load_base_addr_) : mbi2_tag(TYPE), load_base_addr(load_base_addr_) {}
...
struct mbi2_elf_symbols : mbi2_tag
{
    ...
    mbi2_elf_symbols() : mbi2_tag(TYPE) {}
...
struct mb2_header_tag
{
    ...
    explicit mb2_header_tag(uint32_t t) : type(t) {}
    mb2_header_tag(uint32_t t, uint32_t s) : type(t), size(s) {}
...
struct mb2_header_keep_bs : mb2_header_tag
{
    ...
    mb2_header_keep_bs() : mb2_header_tag(TYPE) {}
};
...
struct mb2_header_efi_64bit_entry : mb2_header_tag
{
    ...
    mb2_header_efi_64bit_entry() : mb2_header_tag(TYPE) {}
...
struct mb2_header_relocatable : mb2_header_tag
{
    ...
    mb2_header_relocatable() : mb2_header_tag(TYPE) {}
...
{
    explicit mb2_reader_generic(const uint8_t* raw_pointer) : raw(raw_pointer)
    {
        FIXED_PART fix;
    ...
    struct mb2_iterator_tag
    {
        ...
        explicit mb2_iterator_tag(const uint8_t* addr_) : addr(addr_) {}
    ...
    public:
        ...
        explicit iterator(const uint8_t* addr) : current_tag(addr) {}
        ...
        bool operator!=(const iterator& o) const { return o.current_tag != current_tag; }
        bool operator==(const iterator& o) const { return o.current_tag == current_tag; }
    ...
    iterator begin() const { return iterator(raw + sizeof(FIXED_PART)); }
    iterator end() const { return iterator(raw + size); }
