...
{
public:
    explicit heap_block_manager(math::order_t max_ord_) : max_ord(max_ord_)
    {
     ...
     */
    void mark_used(const free_block_id& block) { free_blocks[block.ord].erase(block.it); }
...
public:
    struct free_block_id
    {
        ...
        math::order_t get_ord() const { return ord; }
        uintptr_t get_addr() const { return *it; }
    };
};
...
public:
     ...
     */
    explicit buddy_impl(uint64_t max_ord) : max_order(max_ord), blocks(max_order)
    {
