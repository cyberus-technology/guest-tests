...
struct __PACKED__ xhci_dbc_capability : xhci_device::capability
{
   ...
   {
      return control & CTRL_RUNNING;
   }

   uint8_t dcerst_max() const
   {
      return (specific() >> DCERST_MAX_SHIFT) & math::mask(DCERST_MAX_BITS);
   }

   uint8_t max_burst_size() const
   {
      return (control >> MAX_BURST_SIZE_SHIFT) & math::mask(MAX_BURST_SIZE_BITS);
   }

   void enable()
   {
      control |= CTRL_ENABLE | CTRL_STATUS_EVENT;
   }
   void disable()
   {
      control &= ~(CTRL_ENABLE | CTRL_STATUS_EVENT);
   ...
   bool run_change() const
   {
      return control & CTRL_RUN_CHANGE;
   }
   void clear_run_change()
   {
      control |= CTRL_RUN_CHANGE;
   }

   void clear_port_status_events()
   {
      port_status_control |= PORTSC_STATUS_EVENTS;
   }

   bool event_ring_not_empty() const
   {
      return status & STAT_ER;
   }

   void ring_doorbell_out()
   {
      doorbell = 0u << DOORBELL_SHIFT;
   }
   void ring_doorbell_in()
   {
      doorbell = 1u << DOORBELL_SHIFT;
   }
