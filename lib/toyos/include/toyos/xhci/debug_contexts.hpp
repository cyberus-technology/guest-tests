...
struct __PACKED__ dbc_endpoint_context
{
    ...
    dbc_endpoint_context() { data.fill(0); }

    uint8_t type() const { return (settings >> TYPE_SHIFT) & math::mask(TYPE_BITS); }
    void type(uint8_t t)
    {
    ...
    void set_dequeue_ptr(uintptr_t ptr) { dequeue_ptr = ptr | PRODUCER_CYCLE_STATE; }
