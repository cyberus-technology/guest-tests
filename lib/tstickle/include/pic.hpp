...
public:
    explicit pic(uint8_t vector_base) : vector_base_(vector_base)
    {
    ...
    bool is_pic_vector(uint8_t vec) { return cbl::interval::from_size(vector_base_, 16).contains(vec); }

    bool is_master_vector(uint8_t vec) { return cbl::interval::from_size(vector_base_, 8).contains(vec); }

    bool is_slave_vector(uint8_t vec) { return cbl::interval::from_size(vector_base_ + 8, 8).contains(vec); }
    ...
    uint16_t get_irr() { return get_irq_reg(ICW_IRR); }

    uint16_t get_isr() { return get_irq_reg(ICW_ISR); }
