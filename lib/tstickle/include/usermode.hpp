...
public:
    ...
    void enter_iret(bool emulated = false, uint64_t set_flags = 0, uint64_t clear_flags = 0)
    {
        asm volatile("mov %%rsp, %%rbx;"
                     ...
                     "1:"
                     :
                     : [emul] "r"(emulated), [dbgport] "i"(DEBUG_PORTS.a),
                       "a"(DEBUGPORT_EMUL_ONCE), [set_flags] "rm"(set_flags), [clear_mask] "rm"(~clear_flags)
                     : "rbx", "rcx", "memory");
    }
    ...
    void leave_syscall()
    {
        ...
        asm volatile("pushf; lea 1f, %%rsi; syscall; 1: popf" ::"D"(0) : "rsi", "rcx", "r11", "memory");
    }

private:
    alignas(PAGE_SIZE) uint8_t kernel_stack_[KERNEL_STACK_SIZE];
    void enable_sce() { wrmsr(x86::EFER, rdmsr(x86::EFER) | x86::EFER_SCE); }
};
