...
public:
    ...
    bdf_t(uint8_t bus_, uint8_t device_, uint8_t function_) : bus(bus_), device(device_), function(function_) {}

    /// Constructs an instance for a given RID.
    explicit bdf_t(uint16_t rid)
        : bus(rid >> BUS_SHIFT)
        , device((rid >> DEVICE_SHIFT) & math::mask(DEVICE_WIDTH))
        , function((rid >> FUNCTION_SHIFT) & math::mask(FUNCTION_WIDTH))
    {}

    /// Constructs an instance from an offset into the MMCONFIG space.
    explicit bdf_t(uintptr_t offset)
        : bus((offset >> (MMCFG_OFFSET + BUS_SHIFT)) & math::mask(BUS_WIDTH))
        , device((offset >> (MMCFG_OFFSET + DEVICE_SHIFT)) & math::mask(DEVICE_WIDTH))
        , function((offset >> (MMCFG_OFFSET + FUNCTION_SHIFT)) & math::mask(FUNCTION_WIDTH))
    {}
    ...
    bool operator==(const bdf_t& o) const { return o.bus == bus and o.device == device and o.function == function; }

    /// Compares this BDF to o, negating the equals operator.
    bool operator!=(const bdf_t& o) const { return not operator==(o); }
