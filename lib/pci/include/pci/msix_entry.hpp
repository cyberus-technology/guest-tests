...
struct msix_entry
{
    ...
    msix_entry() : msg_addr(0), msg_addr_hi(0), msg_data(0), ctrl(IS_MASKED) {}
    ...
    bool masked() const volatile { return ctrl & IS_MASKED; }

    /// Returns the entry's complete 64-bit MSI address (the upper 32 bits might be 0).
    uint64_t msi_address() const volatile { return uint64_t(msg_addr_hi) << 32 | msg_addr; }

    /// Returns the entry's MSI data.
    uint64_t msi_data() const volatile { return msg_data; }

    void mask() volatile { ctrl |= IS_MASKED; }    ///< Masks this entry.
    void unmask() volatile { ctrl &= ~IS_MASKED; } ///< Unmasks this entry.
    ...
    /// Sets the MSI data.
    void msi_data(uint64_t data) volatile { msg_data = data; }
