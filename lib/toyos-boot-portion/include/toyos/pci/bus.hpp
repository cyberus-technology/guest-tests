...
class pci_bus
{
    ...
    public:
        ...
        iterator(phy_addr_t cfg_base, phy_addr_t cfg_end) : cfg_base_(cfg_base), cfg_end_(cfg_end) {}

        pci_device operator*() const { return pci_device(cfg_base_, bdf_); }
        bool operator!=(const iterator& o) const { return o.cfg_base_ != cfg_base_; }
        bool operator==(const iterator& o) const { return o.cfg_base_ == cfg_base_; }
...
public:
     ...
     */
    pci_bus(phy_addr_t mcfg_base, size_t busses) : mcfg_base_(mcfg_base), pci_busses_(busses) {}

    /// Returns begin iterator.
    iterator begin() const { return iterator(mcfg_base_, mcfg_end()); }

    /// Returns end iterator.
    iterator end() const { return iterator(mcfg_end(), mcfg_end()); }
