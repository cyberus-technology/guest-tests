...
struct bar_t_internal
{
    ...
    bar_t_internal(uint32_t low, uint32_t high) : raw(low), raw_hi(high) {}
    ...
    bool is_mem() { return type((static_cast<uint32_t>(raw) & TYPE_MASK)) == type::MEM; }

    /// Returns if the BAR is an I/O BAR.
    bool is_pio() { return type((static_cast<uint32_t>(raw) & TYPE_MASK)) == type::IO; }
...
private:
    ...
    struct access_proxy
    {
        access_proxy(uint32_t val) { WRITE_FN(&value, val); }

        explicit constexpr operator uint32_t() const { return READ_FN(&value); }
