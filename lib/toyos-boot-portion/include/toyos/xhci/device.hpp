...
protected:
    ...
    {
    public:
        port_iterator(const xhci_device& dev, uint8_t port) : dev_(dev), port_(port) {}

        portsc_t* operator*() const { return dev_.get_portsc(port_); }

        bool operator!=(const port_iterator& o) const { return o.port_ != port_; }
    ...
    {
    public:
        explicit port_list(const xhci_device& dev) : dev_(dev) {}
        port_iterator begin() const { return {dev_, 1u}; }

        port_iterator end() const { return {dev_, uint8_t(dev_.get_hcsparams1()->max_ports() + 1u)}; }

    private:
        const xhci_device& dev_;
    };

    port_list get_ports() const { return port_list(*this); }
    ...
    public:
        ...
        bool operator!=(const capability_iterator& o) const { return o.cap_off_ != cap_off_; }
    ...
    {
    public:
        capability_list(const xhci_device& dev, phy_addr_t mmio_base) : dev_(dev), mmio_base_(mmio_base) {}
        capability_iterator begin() const { return {mmio_base_, dev_.get_hccparams1()->ext_cap_offset_dwords()}; }

        capability_iterator end() const { return {mmio_base_, 0}; }
    ...
    capability_list get_capabilities() const { return capability_list(*this, mmio_base_); }
    ...
    hccparams1* get_hccparams1() const { return get_reg<hccparams1>(); }
    hcsparams1* get_hcsparams1() const { return get_reg<hcsparams1>(); }
