...
public:
    ...
    bool full() const { return check_enqueue_increment(enqueue_ptr) == dequeue_ptr; }

    /// \return True, if the queue is empty, False otherwise.
    bool empty() const { return enqueue_ptr == dequeue_ptr; }

    /// \return The index of a given pointer to a TRB (linear address)
    size_t index(uintptr_t ptr) const { return (ptr - ptr_to_num(ring_.data())) / sizeof(trb); }
    ...
    uintptr_t get_dequeue_ptr() const { return dequeue_ptr; }
    ...
    void dump() const
    {
        ...
        for (const auto& trb : ring_) {
            ...
            info("[{02}] {#02x} BUF {#x} CYC {} {s} {s}{s}", index(ptr), trb.type(), trb.buffer, trb.cycle(),
                 trb.type() == trb_link::TYPE ? "LINK" : "", ptr == enqueue_ptr ? " <-- ENQ" : "",
                 ptr == dequeue_ptr ? " <-- DEQ" : "");
        }
