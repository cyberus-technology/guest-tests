...
PDE PDE::pde_to_pt(pd_entry_t config)
{
    config.dirty = false;
...
PDE PDE::pde_to_2mb_page(pd_entry_t config)
{
    config.pagesize = true;
    return PDE(config);
}

PDE::PDE(const pd_entry_t& config)
{
    uint64_t raw = 0;
...
void PDE::set_present(bool pres, tlb_invalidation invl)
{
...
void PDE::set_usermode(bool mode, tlb_invalidation invl)
{
    access_helper(mode, US_MASK, invl);
}

void PDE::set_pwt(bool pwt, tlb_invalidation invl)
{
    access_helper(pwt, PWT_MASK, invl);
}

void PDE::set_pcd(bool pcd, tlb_invalidation invl)
{
    access_helper(pcd, PCD_MASK, invl);
}

void PDE::set_accessed(bool acc, tlb_invalidation invl)
{
...
void PDE::set_exec_disable(bool exec, tlb_invalidation invl)
{
...
void PDE::access_helper(bool value, uint64_t mask, tlb_invalidation invl)
{
