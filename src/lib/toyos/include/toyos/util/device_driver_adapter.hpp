 ...
 public:
    ...
    class mutex_interface
    {
     public:
        ...
        {
         public:
            explicit mutex_guard(mutex_interface& mtx)
                : mtx_(mtx)
            {
                mtx_.acquire();
            }
    ...
    struct dma_buffer_t
    {
         ...
         */
        uintptr_t dma_address(uintptr_t ptr) const
        {
            return dma_address(num_to_ptr<void>(ptr));
        }

     private:
        cbl::interval dma_range() const
        {
            return cbl::interval::from_size(dma_addr, pages * PAGE_SIZE);
        }
        cbl::interval lin_range() const
        {
            return cbl::interval::from_size(uintptr_t(lin_addr), pages * PAGE_SIZE);
        }
    };
...
class dummy_driver_adapter final : public device_driver_adapter
{
 public:
    ...
    /// Dummy allocator.
    virtual dma_buffer_t allocate_pages(size_t) override
    {
        return {};
    }
    /// Dummy free.
    ...
    virtual std::unique_ptr<mutex_interface> get_mutex() override
    {
        return std::make_unique<dummy_mutex>();
    }
