...
class pci_device
{
 ...
 public:
     ...
     */
    explicit pci_device(phy_addr_t cfg_base, bdf_t bdf)
        : cfg_base_(cfg_base), bdf_(bdf) {}

    bdf_t bdf() const
    {
        return bdf_;
    }  ///< Returns the BDF.
    phy_addr_t cfg_base() const
    {
        return cfg_base_;
    }  ///< Returns the config space start address.

    /// Returns the config space page as interval.
    cbl::interval cfg_page() const
    {
        return cbl::interval::from_size(static_cast<uintptr_t>(cfg_base()), PAGE_SIZE);
    }

    /// Indicates if the device is valid (first two bytes are not 0 and not 0xFFFF).
    bool is_valid() const
    {
        return vendor_id() != 0 and vendor_id() != math::mask(16);
    }

    uint16_t device_id() const
    {
        return read(offset::DEVICE_VENDOR_ID, shifts::DEVICE_ID);
    }  ///< Gets Device ID.
    uint16_t vendor_id() const
    {
        return read(offset::DEVICE_VENDOR_ID, shifts::VENDOR_ID);
    }  ///< Gets Vendor ID.
    uint8_t cap_offset() const
    {
        return read(offset::CAP_PTR);
    }  ///< Gets cap ptr.
    uint8_t class_code() const
    {
        return read(offset::CLASS, shifts::CLASS);
    }  ///< Gets class.
    uint8_t subclass() const
    {
        return read(offset::CLASS, shifts::SUBCLASS);
    }  ///< Gets subclass.
    uint8_t prog_if() const
    {
        return read(offset::CLASS, shifts::PROG_IF);
    }  ///< Gets PROG_IF.

    uint8_t type() const
    {
        return read(offset::HEADER_TYPE, shifts::DEV_TYPE) & masks::DEV_TYPE;
    }  ///< Gets type.
    uint8_t bus_primary() const
    {
        return read(offset::BUS_INFO, shifts::BUS_PRIMARY);
    }  ///< Gets primary bus number.
    ...
    unsigned bar_count() const
    {
        return type() == PCI_TYPE_GENERAL ? PCI_NUM_BARS : PCI_NUM_BARS_BRIDGE;
    }

    /// Indicates if this device is a PCI-to-PCI bridge.
    bool is_bridge() const
    {
        return type() == PCI_TYPE_PCI_BRIDGE;
    }
    ...
    bool is_pci_serial() const
    {
        return class_code() == PCI_CLASS_SIMPLE_COMM and subclass() == PCI_SUBCLASS_SERIAL;
    }
 ...
 protected:
    class bar_iterator
    {
     public:
        ...
        bar_iterator(const pci_device& dev, unsigned bar_index)
            : dev_(dev), bar_index_(bar_index) {}

        /// Returns the BAR pointer for the current index.
        bar_t* operator*() const
        {
            return dev_.bar(bar_index_);
        }

        /// Indicates if the iterator o references a different BAR index.
        bool operator!=(const bar_iterator& o) const
        {
            return o.bar_index_ != bar_index_;
        }
    ...
    class capability_iterator
    {
     ...
     public:
        ...
        capability_iterator(const pci_device& dev, uint8_t cap_off)
            : dev_(dev), off(cap_off ?: CAP_OFFSET_INVALID) {}

        /// Returns the capability for the current offset.
        capability operator*() const
        {
            return *reinterpret_cast<capability*>(uintptr_t(dev_.cfg_base()) + off);
        }
 ...
 public:
    ...
    {
     public:
        explicit bar_list(const pci_device& dev)
            : dev_(dev) {}

        bar_iterator begin() const
        {
            return { dev_, 0 };
        }
        bar_iterator end() const
        {
            return { dev_, bar_iterator::BAR_INDEX_INVALID };
        }
    ...
    bar_list get_bars() const
    {
        return bar_list(*this);
    }
    ...
    class capability_list
    {
     public:
        ...
        explicit capability_list(const pci_device& dev)
            : dev_(dev) {}

        capability_iterator begin() const
        {
            return { dev_, dev_.cap_offset() };
        }  ///< Begin iterator.
    ...
    size_t find_msi_cap() const
    {
        return find_cap(msi_capability::ID);
    }  ///< Finds the MSI capability offset.
    size_t find_msix_cap() const
    {
        return find_cap(msix_capability::ID);
    }  ///< Finds the MSI-X capability offset.
 ...
 private:
    ...
    uint32_t read(offset off) const
    {
        return *ptr<const uint32_t>(uintptr_t(off));
    }
    void write(offset off, uint32_t value)
    {
        *ptr<uint32_t>(uintptr_t(off)) = value;
    }

    uint32_t read(offset off, shifts shift) const
    {
        return read(off) >> size_t(shift);
    }
