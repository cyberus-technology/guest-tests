...
#pragma pack(push, 1)
   ...
   struct msi_capability : capability
   {
      ...
      {
         return is_64bit() ? 14 : 10;
      }

      /// Returns if the cap is 64-bit capable.
      bool is_64bit() const volatile
      {
         return msg_ctrl & IS_64B_CAPABLE;
      }

      /// Returns if individual vectors of a multi-message MSI can be masked.
      bool is_pervector_maskable() const volatile
      {
         return msg_ctrl & IS_PERVECTOR_MASKABLE;
      }

      /// Returns if the MSI capability is enabled.
      bool enabled() const volatile
      {
         return msg_ctrl & IS_ENABLED;
      }

      /// Returns the number of bits that comprise the multi-message MSI range.
      uint8_t order() const volatile
      {
         return (msg_ctrl & ORDER_MASK) >> ORDER_SHIFT;
      }

      /// Returns the complete MSI address, honoring 32/64-bit.
      uint64_t msi_addr() const volatile
      {
         return is_64bit() ? (uint64_t(msg_addr_hi) << 32) | msg_addr : msg_addr;
      }

      /// Returns the MSI data field, honoring 32/64-bit.
      uint16_t msi_data() const volatile
      {
         return is_64bit() ? msg_data_64b : msg_data;
      }
      ...
      // compiler warnings.
      msi_capability(const msi_capability& other)
      {
         (*this) = other;
      }
   ...
   struct msix_capability : capability
   {
      ...
      uint8_t table_bar() const volatile
      {
         return offset_table & BAR_MASK;
      }

      /// Returns the offset at which the MSI-X table resides within the BAR.
      size_t table_offset() const volatile
      {
         return offset_table & ~BAR_MASK;
      }

      /// Returns the number of entries in the MSI-X table.
      size_t table_entries() const volatile
      {
         return (msg_ctrl & TBL_SIZE_MASK) + 1;
      }

      /// Returns the size of the MSI-X table in bytes.
      size_t table_size() const volatile
      {
         return table_entries() * sizeof(msix_entry);
      }

      void enable() volatile
      {
         msg_ctrl |= IS_ENABLED;
      }  ///< Enables the MSI-X capability.
      void disable() volatile
      {
         msg_ctrl &= ~IS_ENABLED;
      }  ///< Disables the MSI-X capability.
   ...
   struct sriov_capability : ext_capability
   {
      ...
      bool vf_enabled() const
      {
         return sriov_control & VF_ENABLED;
      }
