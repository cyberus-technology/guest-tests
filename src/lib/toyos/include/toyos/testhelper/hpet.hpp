...
struct __PACKED__ hpet
{
    ...
    static hpet* get(uintptr_t base = DEFAULT_ADDRESS)
    {
        return num_to_ptr<hpet>(base);
    }
    ...
    struct __PACKED__ timer
    {
     public:
        ...
        bool fsb_capable() const
        {
            return cfg & TMR_CAP_FSB;
        }  ///< Returns whether the timer can generate MSIs.
        bool fsb_enabled() const
        {
            return cfg & TMR_FSB_ENABLE;
        }  ///< Returns whether MSI delivery is enabled.
        bool periodic() const
        {
            return cfg & TMR_INT_PERIODIC;
        }  ///< Returns whether the timer is set to periodic.
        ...
        uint8_t ioapic_gsi() const
        {
            return (cfg >> TMR_ROUTE_SHIFT) & math::mask(TMR_ROUTE_BITS);
        }
        ...
        void int_enabled(bool i)
        {
            configure_field(&cfg, TMR_INT_ENABLE, i);
        }

        /// Sets the periodic flag to the value of p.
        void periodic(bool p)
        {
            configure_field(&cfg, TMR_INT_PERIODIC, p);
        }

        /// Sets the MSI flag to the value of e.
        void fsb_enabled(bool e)
        {
            configure_field(&cfg, TMR_FSB_ENABLE, e);
        }
    ...
    {
        return extract(cap_id, CAP_VENDOR_BITS, CAP_VENDOR_SHIFT);
    }

    /// Reads the number of available timers.
    size_t timer_count() const
    {
        return extract(cap_id, CAP_TMR_COUNT_BITS, CAP_TMR_COUNT_SHIFT);
    }

    /// Globally enables/disables the HPET device according to e.
    void enabled(bool e)
    {
        configure_field(&cfg, CFG_ENABLED, e);
    }

    /// Sets the legacy replacement route option to the value of l.
    void legacy_enabled(bool l)
    {
        configure_field(&cfg, CFG_LEGACY, l);
    }

    /// Returns a pointer to the timer n.
    timer* get_timer(size_t n) const
    {
        return num_to_ptr<timer>(ptr_to_num(this) + Tn_BASE + n * Tn_CFG_OFFSET);
    }
    ...
    uint64_t milliseconds_to_ticks(uint64_t ms) const
    {
        return microseconds_to_ticks(1000 * ms);
    }
    ...
    bool interrupt_active(uint8_t timer_no) const
    {
        return int_status & (1u << timer_no);
    }

    /// Clears the active IRQ flag for the given timer.
    void clear_irq(uint8_t timer_no)
    {
        int_status |= (1u << timer_no);
    }
