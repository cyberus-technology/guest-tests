 ...
 */
    class mbi2_builder
    {
     public:
        ...
        void add_system_table(void* system_table)
        {
            add_tag(mbi2_efi_system_table(uint64_t(system_table)));
        }

        void add_image_load_base(uint32_t load_base)
        {
            add_tag(mbi2_image_load_base(load_base));
        }
        ...
        std::vector<uint8_t> build()
        {
            ...
            auto tags_size{ std::accumulate(raw_data.begin(), raw_data.end(), 0, [](int sum, const auto& item) { return sum + item.size(); }) };
