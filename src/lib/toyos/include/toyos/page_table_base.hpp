...
protected:
   ...
   explicit paging_entry_base(uint64_t raw)
      : raw_(raw) {}
   explicit paging_entry_base(const paging_entry_base& other)
   {
...
public:
   ...
   {
      return is_present() and (raw() & US_MASK);
   }
   bool is_pwt() const
   {
      return is_present() and (raw() & PWT_MASK);
   }
   bool is_pcd() const
   {
      return is_present() and (raw() & PCD_MASK);
   }
   bool is_accessed() const
   {
      return is_present() and (raw() & A_MASK);
   }
   bool is_exec_disable() const
   {
      return is_present() and (raw() & XD_MASK);
   }
   ...
   bool operator==(const paging_entry_base& other)
   {
...
protected:
   ...
   uint64_t raw() const
   {
      return cbl::in_place_atomic<const uint64_t>(raw_).load();
   }
   uint64_t raw_;
};
     ...
     unmovable
{
   ...
   iterator_t begin()
   {
      return internal_table.begin();
   }
   iterator_t end()
   {
      return internal_table.end();
   }

   iterator_t cbegin() const
   {
      return internal_table.cbegin();
   }
   iterator_t cend() const
   {
      return internal_table.cend();
   }

   ENTRY& at(size_t idx)
   {
      return internal_table.at(idx);
   }
   ENTRY& operator[](size_t idx)
   {
      return internal_table[idx];
   }
